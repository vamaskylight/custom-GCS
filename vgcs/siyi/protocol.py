"""SIYI Gimbal Camera SDK frame encoding/decoding (UDP/TCP port 37260)."""
...
def crc16(data: bytes, init: int = 0) -> int:
    ...
    for b in data:
        temp = (crc >> 8) & 0xFF
        crc = ((crc << 8) ^ _CRC16_TAB[(b ^ temp) & 0xFF]) & 0xFFFF
    return crc
...
def build_request(cmd_id: int, data: bytes = b"", *, seq: int = 0) -> bytes:
    header = struct.pack(
        "<HBHHB",
        STX,
        CTRL_NEED_ACK,
...
def parse_frame(payload: bytes) -> SiyiFrame | None:
    if len(payload) < 10:
        return None
    if struct.unpack_from("<H", payload, 0)[0] != STX:
        return None
    ctrl, data_len, seq, cmd_id = struct.unpack_from("<BHHB", payload, 2)
    end = 8 + int(data_len)
    if len(payload) < end + 2:
        return None
    body = payload[:end]
    recv_crc = struct.unpack_from("<H", payload, end)[0]
    if crc16(body) != recv_crc:
        return None
    return SiyiFrame(ctrl=int(ctrl), seq=int(seq), cmd_id=int(cmd_id), data=body[8:])


def decode_attitude_deg(data: bytes) -> tuple[float | None, float | None, float | None]:
    """int16 yaw/pitch/roll × 0.1° (SIYI manual §3.5)."""
    if len(data) < 6:
        return None, None, None
    yaw_i, pitch_i, roll_i = struct.unpack_from("<hhh", data, 0)

    def _deg(v: int) -> float:
        return float(v) / 10.0

    return _deg(yaw_i), _deg(pitch_i), _deg(roll_i)
